...
namespace Presentacion.Proveedor
{
    public partial class Proveedor_Actualizar : Form
    {
        ...
        public void LleComReg() // Llenar combobox región
        {
            ...
            CBReg.DataSource = DRegion.Instancia.Listar(); // Listar regiones
        }

        private void CargaCBPro() // Cargar provincias
        {
            int IdReg = Convert.ToInt32(CBReg.SelectedValue);
            DataTable dt = DProvincia.Instancia.Filtrar(IdReg); // Filtrar por región
        ...
        private void CargaCBCom() // Cargar comunas
        {
            ...
            DataTable dt = DComuna.Instancia.Filtrar(IdPro); // Filtrar por provincia
        ...
        {
            List<EComuna> Listar = DComuna.Instancia.Listar(); // Listar objetos de comunas
        ...
        private void ButMod_Click(object sender, EventArgs e) // Al clickear modificar
        {
            ...
            {
                foreach (EProveedor item in DProveedor.Instancia.Listar()) // Buscar rut en proveedores
                {
                    if (item.Rut == Ent.Rut)
                    {
                ...
                bool resultado = DProveedor.Instancia.Actualizar(Ent);
