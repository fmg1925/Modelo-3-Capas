...
namespace Presentacion.Proveedor
{
    public partial class Proveedor_Ingresar : Form
    {
        ...
        public void LleComReg() // Llenar combo box región
        {
            ...
            RegionComboBox.DataSource = DRegion.Instancia.Listar();
        ...
        private void CargaProvinciaComboBox() // Cargar datos en provincia combo box
        {
            ...
            DataTable dt = DProvincia.Instancia.Filtrar(IdRegion);
        ...
        private void CargaComunaComboBox() // Cargar datos en comuna combo box
        {
            ...
            DataTable dt = DComuna.Instancia.Filtrar(IdProvincia);
        ...
        {
            if ((RutTextBox.Text.Trim() != "") && (NombreTextBox.Text.Trim() != "") && (RegionComboBox.Text.Trim() != "") && (ProvinciaComboBox.Text.Trim() != "") && (ComunaComboBox.Text.Trim() != "") && (DireccionTextBox.Text.Trim() != "") && (TelefonoTextBox.Text.Trim() != "") && (EmailTextBox.Text.Trim() != "") && (GiroTextBox.Text.Trim() != "") && (DescripciónTextBox.Text.Trim() != "") && RutTextBox.Text != "Rut Inválido") // Si ningún texto está vacío y el rut es válido
            {
        ...
        private void IngresarButton_Click(object sender, EventArgs e) // Al clickear ingresar
        {
            ...
            foreach (EProveedor item in DProveedor.Instancia.Listar()) // Buscar rut en clientes
            {
                if (item.Rut == Ent.Rut)
                {
                    MessageBox.Show("Ya hay un proveedor existente con ese rut", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error); // Reclamar que el rut ya existe
                    return; // Cancelar ingreso
                }
            }
            if (res == DialogResult.Yes)
            {
                bool resultado = DProveedor.Instancia.Ingresar(Ent); // Ingresar proveedor

                if (resultado) // Si se ingresó
                {
