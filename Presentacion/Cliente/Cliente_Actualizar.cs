...
namespace Presentacion.Cliente
{
    public partial class Cliente_Actualizar : Form
    {
        ...
        private List<ECliente> Listar() // Llamar al procedimiento listar
        {
        ...
        private void ButMod_Click(object sender, EventArgs e) // Al clickear modificar
        {
            ...
            {
                foreach (ECliente item in DCliente.Instancia.Listar()) // Buscar rut en clientes
                {
                    if (item.Rut == Ent.Rut)
                    {
                ...
                bool resultado = DCliente.Instancia.Actualizar(Ent); // Actualizar
                if (resultado) // Si se actualizó
                {
